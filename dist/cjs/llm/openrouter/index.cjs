...
class ChatOpenRouter extends index.ChatOpenAI {
     ...
     */
    _convertOpenAIChatCompletionMessageToBaseMessage(
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    message, 
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    rawResponse) {
        // First call parent to get the base message
        const baseMessage = super._convertOpenAIChatCompletionMessageToBaseMessage(message, rawResponse);
    ...
    async *_streamResponseChunks2(messages$1, options, runManager) {
        ...
        for await (const data of streamIterable) {
            ...
            if (typeof chunk.content !== 'string' && !Array.isArray(chunk.content)) {
                ...
                console.log('[WARNING]: Received non-string/non-array content from OpenAI. This is currently not supported.');
                continue;
            }
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ...
            await runManager?.handleLLMNewToken(textContent, newTokenIndices, undefined, undefined, undefined, { chunk: generationChunk });
        }
        if (usage) {
